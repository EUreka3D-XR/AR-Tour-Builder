...
const getRelativeTime = (dateString, t) => {
  ...
  if (diffInDays === 1)
    return t("dashboard.recentTours.relativeTime.oneDayAgo");
  if (diffInDays < 7)
    return t("dashboard.recentTours.relativeTime.daysAgo", {
      days: diffInDays,
    });

  const diffInWeeks = Math.floor(diffInDays / 7);
  if (diffInWeeks === 1)
    return t("dashboard.recentTours.relativeTime.oneWeekAgo");
  if (diffInWeeks < 4)
    return t("dashboard.recentTours.relativeTime.weeksAgo", {
      weeks: diffInWeeks,
    });
    ...
    <List disablePadding dense>
      {tours?.map((tour) => (
        <TourListItem
          key={tour.id}
          disablePadding
          onClick={() => navigate(routes.tours.one(tour.id))}
        >
          <div>
            ...
            <div className="tour-meta">
              <div className="tour-meta-item">
                ...
                <span>
                  {`${tour.totalPois || tour.pois?.length || 0} ${t("dashboard.recentTours.pois")}`}
                </span>
              </div>
              <div className="tour-meta-item">
              ...
              <div className="tour-meta-item">
                <span>
                  {getRelativeTime(tour.updatedAt || tour.createdAt, t)}
                </span>
              </div>
            </div>
          </div>
            ...
            <Chip
              label={
                tour.status === "published"
                  ? t("dashboard.recentTours.status.published")
                  : t("dashboard.recentTours.status.draft")
              }
